...
const REACTUS_ORIGIN = (
  process.env.NEXT_PUBLIC_REACTUS_BASE_URL ??
  process.env.REACTUS_BASE_URL ??
  ''
).replace(/\/$/, '');
...
export function HappySeedsWatermark() {
  ...
  useEffect(() => {
    try {
      ...
      // Still show watermark when opened as a standalone preview URL
      // (e.g. preview.happyseeds.space) even if technically inside a frame.
      const isStandalonePreview =
        window.location.hostname.includes('happyseeds.space') ||
        window.location.hostname.includes('happyseeds.ai') ||
        !inIframe;
      setIsIframe(inIframe && !isStandalonePreview);
    } catch {
      setIsIframe(false);
    }
    setReady(true);
